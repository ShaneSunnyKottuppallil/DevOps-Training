...
get_subnet_id() {
  ...
  aws ec2 describe-subnets \
    --filters "Name=tag:Name,Values=${name}" "Name=vpc-id,Values=${vpcid}" \
    --query "Subnets[0].SubnetId" \
    --output text
}


pubsubaid=$(get_subnet_id "PublicSubnetA2")
pubsubbid=$(get_subnet_id "PublicSubnetB2")
pubsubcid=$(get_subnet_id "PublicSubnetC2")

prisubaid=$(get_subnet_id "PrivateSubnetA2")
prisubbid=$(get_subnet_id "PrivateSubnetB2")
prisubcid=$(get_subnet_id "PrivateSubnetC2")
...
# ChatAppA2 in private subnet 
aws ec2 run-instances --image-id "$ec2amiid" --instance-type t3.micro --key-name "chatapp" --subnet-id "$prisubaid" --security-group-ids "$chatappsgcliid" --count 1 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=ChatAppA2}]' 
